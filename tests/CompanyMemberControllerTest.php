...
class CompanyMemberControllerTest extends OrchestraTestCase
{
    ...
    public function test_company_member_permissions_can_be_updated()
    {
        ...
        $response = $this->actingAs($company->owner)->put('/companies/'.$company->id.'/members/'.$adam->id, [
            'role' => 'editor',
        ]);
    ...
    public function test_company_member_permissions_cant_be_updated_if_not_authorized()
    {
        $this->migrate();
        ...
        $response = $this->actingAs($adam)->put('/companies/'.$company->id.'/members/'.$adam->id, [
            'role' => 'admin',
        ]);
